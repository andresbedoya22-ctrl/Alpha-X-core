...
import pandas as pd
from scripts.run_policy_stress import main
...
def test_run_policy_stress_main_exports_report(
    tmp_path: Path,
    monkeypatch: object,
    capsys: object,
) -> None:
    ...
    exit_code = main(["--run-id", "test-policy-stress-run"])
    output = capsys.readouterr().out

    assert exit_code == 0
    assert "Periodo test usado:" in output
    assert "Stress summary por variante:" in output
    assert "Estabilidad por subtramo:" in output
    assert "Conclusion:" in output
    assert (reports_dir / "policy_stress" / "test-policy-stress-run" / "summary.json").exists()
    assert (
        reports_dir / "policy_stress" / "test-policy-stress-run" / "stress_summary.csv"
    ).exists()
    assert (
        reports_dir / "policy_stress" / "test-policy-stress-run" / "subperiod_stability.csv"
    ).exists()
